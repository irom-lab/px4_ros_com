...
class Physics(Enum):
    ...
    # PyBullet physics update with aerodynamic drag forces
    PYB_WIND_AERO_DRAG = "pyb_wind_aero_drag"
    # PyBullet physics update with blade flapping external moments
    PYB_WIND_BF_DRAG = "pyb_wind_bf_drag"
    # PyBullet physics update with aerodynamic drag forces and blade flapping external moments
    PYB_WIND_AERO_BF_DRAG = "pyb_wind_aero_bf_drag"
...
class BaseAviary(gym.Env):
    ...
    def __init__(
        ...
        # wind
        wind_model='simple',
        # used in _drag function (to be depreciated)
        wind_force=[0, 0, 0],
        wind_vector=np.array(
            [1.1, 0, 0]).reshape(
            (3, 1)),  # used in _wind_aero_... functions
    ):
        ...
        # Blade scaled natural frequency (listed as 1.9 on p.4 of 2016, 1.5 in 2020
        self.nu_beta = 1.9
        ...
        # approximation, induced angle of attack [rad]
        self.alpha_ind = np.arctan(self.lambda0/0.75)
        # effective angle of attack [rad]
        self.alpha_eff = self.th0 + (3/4)*self.thtw - self.alpha_ind
        # rotor blade length [m] ##NOT SURE IF ROTOR BLADE LENGTH = ROTOR RADIUS !!
        self.rbar = 0.0635
        ...
        self.MAX_RPM = np.sqrt(
            (self.THRUST2WEIGHT_RATIO * self.GRAVITY) /
        ...
        if self.VISION_ATTR:
            ...
            self.IMG_CAPTURE_FREQ = int(
                self.SIM_FREQ / self.IMG_FRAME_PER_SEC)
            self.rgb = np.zeros(
        ...
        if self.GUI:
            ...
            if self.RECORD:
                ...
                self.CAPTURE_FREQ = int(
                    self.SIM_FREQ / self.FRAME_PER_SEC)
    ...
    def step(
        ...
        if self.USE_GUI_RPM:
            ...
            if self.step_counter % (self.SIM_FREQ / 2) == 0:
                self.GUI_INPUT_TEXT = [
                    p.addUserDebugText(
                        "Using GUI RPM", textPosition=[0, 0, 0],
                        textColorRGB=[1, 0, 0],
        ...
        for _ in range(self.AGGR_PHY_STEPS):
            ...
            for i in range(self.NUM_DRONES):
                if self.PHYSICS == Physics.PYB:
                    ...
                    self._physics(clipped_action[i, :], i)
                    self._wind_aero_drag(
                        self.last_clipped_action[i, :],
                        i, self.WIND_VECTOR)  # clipped vs. last clipped?
    ...
    def _startVideoRecording(self):
        ...
        if self.RECORD and self.GUI:
            if self.VIDEO_PATH is not None:
                ...
                # os.path.dirname(os.path.abspath(__file__))+"/../../files/videos/video-"+datetime.now().strftime("%m.%d.%Y_%H.%M.%S")+".mp4"
                fileName = os.path.dirname(os.path.abspath(
                    __file__)) + "/video-" + datetime.now().strftime("%m.%d.%Y_%H.%M.%S") + ".mp4"
    ...
    def _exportImage(self,
        ...
        if img_type == ImageType.RGB:
            ...
            temp = (
                np.sum(img_input[:, :, 0: 2],
                       axis=2) / 3).astype('uint8')
        else:
    ...
    def _wind_aero_drag(self,
        ...
        # Average propeller angular velocity [RPM]
        omega_j = np.mean(rpm)
        ...
        # print("base_rot:")
        # print(base_rot)
        wind_vector = np.matmul(
            np.linalg.inv(base_rot),
            wind_vector_inertial)  # Body Frame
        ...
        # Bluff Body Drag
        f_bluff = 0.5*self.rho*np.linalg.norm(
            wind_vector)*self.Af*self.Cd*wind_vector
        ...
        for i in range(4):
            ...
            f_ind += self.Nb/4*self.rho*self.c*self.Cla*self.alpha_eff*np.sin(
                self.alpha_ind)*omega_j*self.rbar**2*(np.dot(wind_vector.reshape((3,)), U1.reshape((3,))))*U1
    ...
    def _wind_bf_drag(self,
        ...
        # Operational Variables
        omega_j = 8000  # Propeller nominal angular velocity [RPM]
        # Propeller tip speed velocity [m/s]
        v_tip = omega_j*2*np.pi/60*self.rbar
        #### Rotation matrix of the base ###########################
        base_rot = np.array(p.getMatrixFromQuaternion(
            self.quat[nth_drone, :])).reshape(3, 3)
        wind_vector = np.matmul(
        ...
        # Glauert longitudinal inflow gradient (also k_x)
        k_lambda_x = 15*np.pi/23*np.tan(chi/2)

        beta_1c = -self.gamma/(8*(self.nu_beta**2-1)
                               )*self.lambda0*k_lambda_x
        beta_1s = mu*self.gamma/(4*(self.nu_beta**2-1)
                                 )*(4/3*self.th0+self.thtw-self.lambda0)
        ...
        M_single_hub = self.Nb/2*self.k_beta * \
            beta_max*(np.cos(phi_d)*U1+np.sin(phi_d)*U2)
        # Calculate Total Hub Moment
        M_aero = np.array([[self.Nr*self.k_beta*beta_max*np.sin(phi_d)*np.dot(U2.reshape(3,), np.array([1, 0, 0]))], [
                          self.Nr*self.k_beta*beta_max*np.sin(phi_d)*np.dot(U2.reshape(3,), np.array([0, 1, 0]))], [0]])
    ...
    def _wind_frame(self, V_infty: np.ndarray) -> np.ndarray:
        ...
        U3 = np.array(
            [0, 0, 1]).reshape(
            (3, 1))  # U3 = e3 in body coordinates
        U1 = np.zeros([3, 1])
        # Extract horizontal components of V_infty (ignore V_infty[2])
        U1[0:2] = V_infty[0:2]
        if np.linalg.norm(U1) != 0:
            U1 = U1 / np.linalg.norm(U1)  # Normalize U1
        # Determine U2 from cross product
        U2 = -np.cross(U1, U3, axis=0)

        # Horizontal concatenation
        return np.hstack((U1, np.hstack((U2, U3))))
    ...
    def _downwash(self, nth_drone):
        ...
        for i in range(self.NUM_DRONES):
            ...
            if delta_z > 0 and delta_xy < 10:  # Ignore drones more than 10 meters away
                alpha = self.DW_COEFF_1 * (self.PROP_RADIUS /
                                           (4 * delta_z)) ** 2
                beta = self.DW_COEFF_2 * delta_z + self.DW_COEFF_3
                downwash = [
                    0, 0, -alpha * np.exp(-.5 * (delta_xy / beta)**2)]
                p.applyExternalForce(self.DRONE_IDS[nth_drone],
                                     4,
                                     forceObj=downwash,
                                     posObj=[0, 0, 0],
    ...
    def _dynamics(self, rpm, nth_drone):
        ...
        rotation = np.array(
            p.getMatrixFromQuaternion(quat)).reshape(
            3, 3)
        ...
        z_torque = (-z_torques[0] + z_torques[1] - z_torques[2] +
                    z_torques[3])
        if self.DRONE_MODEL == DroneModel.CF2X:
        ...
        torques = torques - np.cross(rpy_rates,
                                     np.dot(self.J, rpy_rates))
        ...
        p.resetBasePositionAndOrientation(
            ...
            pos, p.getQuaternionFromEuler(rpy),
            physicsClientId=self.CLIENT)
    ...
    def _parseURDFParameters(self):
        ...
        """
        URDF_TREE = etxml.parse(os.path.dirname(
            os.path.abspath(__file__)) +
            "/../assets/" + self.URDF).getroot()
